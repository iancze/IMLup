...
def plot_raw(fname, outdir, residual=True):
    ...
    for spw in spws:
        fig = visualization.plot_scatter_datadescid(fname, spw, residual=residual)
        fig.savefig(p / "{:02d}.png".format(spw), dpi=300)

    plt.close("all")


def plot_rescaled_resid(fname, outdir, residual=True):
    ...
    for spw in spws:
        ...
        fig = visualization.plot_scatter_datadescid(fname, spw, sigma_rescale=sigma_rescale, residual=True)
...
def main():
    ...
    if args.scale == "raw":
        plot_raw(args.ms, args.outdir, residual=False)

    if args.scale == "raw-resid":
        plot_raw(args.ms, args.outdir)
        
    if args.scale == "rescale":
        plot_rescaled_resid(args.ms, args.outdir)
